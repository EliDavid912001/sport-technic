...
/** Load src/data/* — repo root (./src/data) or Render layout with server.js inside src/ (./data). */
function requireDataModule(filename) {
  const candidates = [
    path.join(__dirname, 'src', 'data', filename),
    path.join(__dirname, 'data', filename)
  ];
  for (const absPath of candidates) {
    if (fs.existsSync(absPath)) return require(absPath);
  }
  throw new Error(
    "Cannot find module './src/data/" + filename + "' (tried: " + candidates.join(', ') + ')'
  );
}

// --- TrainIQ data layer (exercise registry + AnalysisRecord schema) ---
const { exerciseCatalog } = requireDataModule('exerciseCatalog.js');
const { getExercise, listCatalogForUi } = requireDataModule('exerciseRegistry.js');
const {
  createEmptyAnalysisRecord,
  validateAnalysisRecord,
  ANALYSIS_RECORD_VERSION
} = requireDataModule('dbSchema.js');
